...
# Note: Not using existing theme_manager to avoid dependencies
...
class SettingsDialog:
    ...
    def create_settings_dialog(self):
        ...
        # Main content container - FIXED: Using fixed height instead of adaptive_height
        content = MDBoxLayout(
            orientation='vertical',
            spacing=dp(20),
            size_hint_y=None,
            height=dp(600),  # Fixed height to prevent layout loops
            padding=[dp(20), dp(10)]
        )
    ...
    def toggle_dark_mode(self, instance, value):
        """Toggle dark mode"""
        try:
            # Simple placeholder implementation - not using existing theme manager
            logger.info(f"Dark mode {'enabled' if value else 'disabled'}")
            # Future implementation: could save to config file or preferences
        except Exception as e:

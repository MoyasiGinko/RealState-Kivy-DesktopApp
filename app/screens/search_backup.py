...
# -*- coding: utf-8 -*-
"""
Real Estate Management System - Enhanced Search and Reports Screen
"""
...
class ModernTabContainer(BoxLayout):
    """Modern tab container with enhanced styling"""
...
class SearchScreen(Screen):
    """Enhanced Search and Reports Screen with Modern Design"""
    ...
    def build_modern_ui(self):
        """Build the enhanced modern search and reports UI"""
        ...
        self.build_tabbed_interface(main_layout)
    ...
    def build_header_section(self, main_layout):
        ...
        with header_layout.canvas.before:
            Color(0.6, 0.2, 0.8, 1)  # Purple gradient for search
        ...
        export_btn = BilingualButton(
            translation_key='export_results',
        ...
        header_layout.add_widget(export_layout)
        main_layout.add_widget(header_layout)

    def build_tabbed_interface(self, main_layout):
        """Build enhanced tabbed interface"""
        tabs_container = BoxLayout(
            orientation='vertical',
            spacing=dp(15),
            size_hint_y=None,
            height=dp(600)
        ...
        self.tab_buttons = []
        tab_data = [
            ('advanced_search', [0.2, 0.6, 1, 1]),
            ('reports', [0.8, 0.4, 0.1, 1]),
            ('statistics', [0.1, 0.7, 0.4, 1])
        ]

        for i, (tab_key, color) in enumerate(tab_data):
            tab_btn = BilingualButton(
                translation_key=tab_key,
                background_color=color,
                font_size='16sp',
                size_hint_x=1.0/len(tab_data)
            )
            tab_btn.tab_index = i
            tab_btn.bind(on_press=self.switch_tab)
            tab_buttons_layout.add_widget(tab_btn)
            self.tab_buttons.append(tab_btn)

        tabs_container.add_widget(tab_buttons_layout)

        # Tab content container
        self.tab_content_container = BoxLayout(orientation='vertical')
        tabs_container.add_widget(self.tab_content_container)
        ...
        self.current_tab = 0
        self.switch_tab(self.tab_buttons[0])

        main_layout.add_widget(tabs_container)
    ...
    def refresh_statistics(self):
        ...
        try:
            ...
            self.stats_container.add_widget(self._create_stats_card(
                'Total Properties', str(total_props), [0.2, 0.6, 1, 1]
            ))

            # Total owners
            total_owners = stats.get('total_owners', 0)
            self.stats_container.add_widget(self._create_stats_card(
                'Total Owners', str(total_owners), [0.8, 0.4, 0.1, 1]
            ))
    ...
    def _create_stats_card(self, title: str, value: str, color: list) -> BoxLayout:
        ...
        with card_layout.canvas.before:
            Color(*color)
            card_layout.bg_rect = RoundedRectangle(radius=[12])
            card_layout.bind(
                size=lambda *args: setattr(card_layout.bg_rect, 'size', card_layout.size),
                pos=lambda *args: setattr(card_layout.bg_rect, 'pos', card_layout.pos)
            )

        card_layout.add_widget(RTLLabel(
            text=title,
            font_size='16sp',
            bold=True,
            color=[1, 1, 1, 1]
